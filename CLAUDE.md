...
- `tools/backfill_community_tree/` — one-time script (not a migration) that stamps `tree` onto every existing untreed `news`/`events`/`albums` record (single explicit `--target-tree`, resolved by name, no auto-derivation — unlike persons, there's no natural per-record grouping key here) and repoints the 3 `svc-*` service-account users onto a dedicated, deliberately unlinked "Service Accounts" tree. Idempotent, `--dry-run` supported.
...
- **`news`** — `title`, `body`, `author` (→users), `tree` (→trees). Tree-scoped (see below).
- **`albums`** — `name`, `description`, `year`, `cover_photo` (file), `tree` (→trees). Create/update/delete restricted to `family_admin`; visibility tree-scoped.
- **`photos`** — `album` (→albums, cascade delete), `image_url` (text — see note below), `caption`, `taken_date`, `uploader` (→users), `tagged_persons` (→persons, multi). Update/delete by uploader or admin. No `tree` field of its own — visibility derived via `album.tree`. **Note:** the original `image` file field was supposed to be removed by `1718500012_photos_image_to_url.js` in favor of `image_url`, but that migration used the same removeField-by-name bug documented below, so `image` remained required on any from-scratch install until fixed out-of-band on real dev/prod (never a live issue there, only on fresh disposable containers built purely from migrations).
...
- **`events`** — `name`, `type`, `description`, `start_date`, `end_date`, `location`, `cover_photo` (file), `organizers` (→users, multi), `created_by` (→users), `tree` (→trees). Tree-scoped (see below); write hooks layer on top of the existing `organizers ~ self || family_admin` update/delete rule.
- **`event_rsvps`** — `event` (→events, cascade delete), `user` (→users, cascade delete), `status`. No `tree` field — visibility derived via `event.tree`. Write authorization is `user = self` only (doesn't mention `family_admin` at the rule level — a known, pre-existing gap, not fixed).
- **`comments`** — `body`, `author` (→users), `related_id` (plain text, no FK), `related_type` (select: `photo`/`album` — **singular**, unlike the actual `photos`/`albums` collection names, see the gotcha below). No `tree` field — visibility derived via the referenced photo/album's tree (a `photo`-type comment needs a second hop through its album). Comment *create* has no relation-visibility check today — a known, pre-existing gap (same category as `event_rsvps`' rule gap above), not fixed.
...
**Superuser/admin-authenticated requests bypass everything** (PocketBase's own rule engine already does this; the hooks explicitly no-op when `e.httpContext.get('authRecord')` is null, to match rather than fight that). `tools/gedcom_sync`, `tools/ai_research_mcp`, and `tools/photos_sync` now authenticate as dedicated `family_admin=true` **regular `users` records** (`svc-gedcom-sync@reunion.local` / `svc-ai-research@reunion.local` / `svc-photos-sync@reunion.local`, credentials in 1Password) via `/api/collections/users/auth-with-password`, not PocketBase's superuser `_superusers` table — same effective permissions (family_admin bypasses every hook/rule), but now attributable to named, revocable accounts and actually bound by whatever the rules/hooks say rather than a blanket bypass. These 3 accounts are deliberately repointed onto a dedicated, never-linked "Service Accounts" tree so they don't show up in any real tree's directory. `tools/apple_secret/regenerate.py` (manages OAuth2 provider config) and `tools/backfill_tree`/`tools/backfill_community_tree` (one-time scripts) still use true superuser auth — the former genuinely needs it, the latter are one-off data operations, not recurring services.
...
**Community layer (`news`/`events`/`albums`/`photos`/`event_rsvps`/`comments`) is also tree-scoped**, same fail-open-forever-for-untreed policy as `persons`/`couples`. `news`/`events`/`albums` carry their own `tree` field (write-scoping hooks on `news`/`events` only — `albums`' create/update/delete is already `family_admin`-only at the rule level, so a write hook there would have zero non-bypassed callers). `photos`/`event_rsvps`/`comments` have no `tree` field of their own — visibility is derived at hook time via a relation lookup (`photos.album`→tree, `event_rsvps.event`→tree, `comments.related_id`+`related_type`→ the referenced photo/album's tree, with a photo-type comment needing a *second* hop through its own album). This closed a real reported bug: a user added to a genealogically-unrelated tree could see another family's events/photos/directory entries, because these collections (plus `users` list/view for the directory and home-dashboard member count) were previously either explicitly global by design or simply never filtered.

**`comments.related_type` values are singular (`"photo"`/`"album"`) but the real PocketBase collections are plural (`"photos"`/`"albums"`).** Calling `dao.findRecordById(record.get('related_type'), id)` directly silently fails every lookup — and since lookup failure is fail-open by this codebase's convention, that bug would make *every* comment permanently visible to everyone, with no error and nothing that looks wrong under casual testing. `comments_tree_visibility.pb.js` maps this explicitly inline in every callback (can't factor to a shared helper — the self-containment constraint above applies per-callback, not per-file).

**Two hook files must never both register `onRecordsListRequest`/`onRecordViewRequest` for the same collection.** This codebase has zero precedent for it and whether PocketBase 0.22.20 correctly composes two independent registrations on one event (vs. one silently winning) is unverified. `users_visibility.pb.js` (formerly `users_privacy_redact.pb.js`) carries both phone-redaction *and* tree-scoping in one merged file for exactly this reason, rather than adding a sibling hook — each callback still one self-contained closure, just two concerns in it, the same shape `person_facts_tree_visibility.pb.js` already uses for public-vs-sensitive fact gating.

**A `throw` inside a try/catch — even guarded by an `instanceof` re-throw check — silently swallows the intentional block in this PocketBase version**, confirmed the hard way while building the community-layer hooks (several view-hook drafts wrapped their `NotFoundError` throw this way, matching the exact anti-pattern already flagged in the note above about `NotFoundError`/`ForbiddenError`). The fix: compute a `blocked` boolean inside a try/catch that only ever catches lookup errors and never throws, then throw unconditionally *outside* that block.
...
**Grid/flex items default to `min-width:auto`, which floors their width at their content's min-content size.** A column hosting unpredictable user-submitted text (e.g. a pasted URL with no natural break points, in the event location-suggestion form) can force its track wider than its `fr` share, blowing the whole grid out past its intended max-width. Fixing this needs *both* `min-width:0` on the grid items (lets the track actually shrink to its `fr` share) *and* `overflow-wrap:anywhere` on the specific text node holding the freeform content (so the text itself doesn't bleed past its own box once the track is sized correctly) — confirmed by reproducing the bug with each fix applied in isolation: `min-width:0` alone correctly re-sizes the columns but the raw text still overflows its own card; `overflow-wrap` alone happens to mask the symptom for a single long string but doesn't fix the underlying track-sizing bug for any other cause of overflow. `#main{overflow-x:hidden}` (`app.css`) means this class of bug has no scrollbar to reveal it — content past the visible edge is just silently clipped, so a user report like "I can't see column X, no scrollbar" should point straight at this pattern rather than a missing-content bug.

**Component-specific responsive breakpoints must account for `#sidebar`'s own breakpoint**, not just assume the generic 767px content breakpoint (`@media (max-width:767px)`, used throughout `app.css`) is early enough. `#sidebar` (248px, `position:sticky`) only collapses below that same 767px, so a two-column block that also collapses at exactly 767px still has to share space with a visible 248px sidebar at every width just above that — it can get squeezed far more than a naive per-track `fr` calculation suggests. `.event-detail-columns` uses a dedicated breakpoint for this reason, not the shared 767px one — **and that breakpoint isn't a one-time constant**: it was originally 1023px when the column block spanned a flat 960px, but became 1750px once the block's own width changed to a 75% cap on `.ed-page` (25% of 75% of `#main` is a much smaller absolute pixel budget than 35% of a flat 960px was). Any future change to either the page's width cap or the column `fr` ratio invalidates the existing breakpoint number and requires recomputing it — it does not scale itself.

**Whole-day date-poll slots** (`event.date_poll_slot_minutes === 1440`) are stored as midnight UTC. Every display site for `event.start_date` must pass `fmtEventDate(iso, { allDay: event.date_poll_slot_minutes === 1440 })` (forces UTC formatting, drops the time-of-day component) — otherwise a user west of UTC sees the date shifted back a day (midnight UTC on Aug 1 is still Jul 31 in `America/Los_Angeles`). For the same reason, `renderEventsList`'s upcoming/past split compares all-day events by UTC *calendar date*, not the raw instant against `now` — a plain instant comparison would drop an all-day event for "today" into Past right after midnight UTC, before that date has even begun locally. `saveEvent` resets `date_poll_slot_minutes` back to a normal value whenever the date poll is disabled — otherwise the stale `1440` marker keeps triggering all-day formatting/collapse behavior on an event that's since reverted to an ordinary timed one.

**Any list fetch capped at a single `perPage=200` page needs full pagination once its *count* (not just its content) is surfaced in the UI** — e.g. the availability-poll response counter shown in the collapsed-summary view. A single-page fetch will silently under-report the count past 200 responses with no error. Conversely, skip that fetch/pagination entirely when the render branch that consumes it isn't active (e.g. don't paginate `event_availability` once `date_poll_status !== 'open'`, since a closed poll never renders the grid or the counter) — otherwise a finalized high-attendance event pays for an unbounded number of round trips on every single page visit for data that's never displayed.
